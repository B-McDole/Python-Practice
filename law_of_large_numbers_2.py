...
#Let us quickly discuss "The Law of Large Numbers".  So, without all the
#limit notations and calculus, the law states that as the number of 'trials',
#which in this case means experiments, becomes larger and larger, the 
#empirical probablity will near the theoretical probability.  That somehow
#did not sound less complex at all!  We consider an example:
#If I were to flip a coin, we agree that the probability of heads is .5
#and tails is .5.  So, if I flip 10 coins, one might expect 5 heads, right?
#Well, we know that we will probably not get 5 heads!  What if I flip 100 coins?
#I should get 50 heads or so.  What if I flip 1000 coins?  We continue forever.
#The larger number of coins I flip, the closer I should get to half being heads.
#That is more or less the law of large numbers.
#How in the heck does that apply here?  Well, the more numbers I randomly generate,
#the closer I should get to that 'idea' empirical rule ratio!
#This also gives us the chance to discuss functions!

#def is telling python we want a function.  Inside of the () are the arguments we pass.

def empirical(n):
    list_n = randn(n)
    q0 = 0 #We need our counters, and we want them to start fresh each time we call empirical!
    q1 = 0
    q2 = 0
    q3 = 0
    q4 = 0
    ...
    for i in list_n:
        if i < -2:
            ...
            q4 +=1 #Oh we are fancy.  This is equivalent to q4 = q4+1.  Neat!
        else:
            q5 +=1
    
#Great!  So, now we call the function.
empirical(4)

#Right.  That is a bit anticlimactic.  We did not tell empirical to do anything.
#We need to either have the function print something, or have it return some values.
#I will opt for printing.
...
def empirical_2(n):
    ...
    for i in list_n:
        if i < -2:
            ...
            q5 +=1
    print("Should be roughly: 2.5, 12.5, 34, 34, 12.5, 2.5") 
    print("Actually is:", q0/n*100, q1/n*100, q2/n*100, q3/n*100, q4/n*100, q5/n*100) #Notice the indent is before the q-line.
...
#First, we should probably just have this round.  I think to the tenth.
#Second, the further out we go, the closer we get to the lovely empirical rule!
#It looks like our function is in fact working as intended.  How interesting!

#Is there more we could do with this?  Absolutely!
#Is there more that is worth doing with this?  Kind of!
#Let us look at some visualization.  I would love to track and see if we 
#are actually getting more accurate.  What does our data tell us?
